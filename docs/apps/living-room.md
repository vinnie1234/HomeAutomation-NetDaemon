...
#### Lights Not Responding to Wall Switch
1. **Device ID**: Verify Hue button device ID in configuration
2. **Hue Events**: Check that Home Assistant receives "hue_event" events
3. **Network**: Ensure Hue bridge connectivity
4. **Button Battery**: Check Hue button battery level

#### TV Mode Not Activating
1. **Media Player**: Verify TV entity state changes are detected
2. **Integration**: Check Home Assistant TV integration
3. **Network**: Ensure TV is properly connected to network

#### Gaming Mode Issues
1. **Device Detection**: Verify gaming device entity states
2. **Input Sources**: Check TV input source names match configuration
3. **Audio Settings**: Verify audio entity names and capabilities

#### Inconsistent Lighting
1. **Bulb Connectivity**: Check individual light connectivity
2. **Timing Issues**: Verify delay settings in configuration
3. **State Sync**: Ensure all lights respond to color temperature changes

### Debug Information
Enable debug logging for detailed behavior tracking:
```json
{
    "Logging": {
        "LogLevel": {
            "Automation.apps.Rooms.LivingRoom": "Debug"
        }
    }
}
```